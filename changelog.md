...
## Unreleased
- **Inventory: the buildings a Tome can be opened for are listed.** A Tome sat in the table as one row that said nothing of what was inside it, so weighing one up meant opening it in the game and reading the choices off its window. Each building a Tome offers is now a row of its own, typed "Building (Tome)" and naming the Tome under the building's name, with the size, output, resale value and disenchant value it would have at the chapter the Tome was won in - the chapter the game fills in when the Tome is opened. Searching for the Tome's name finds them too, and "Aggregate by Name" keeps them apart from buildings already in the inventory, so a total is never made up of buildings not yet chosen. There is no Place button on these rows, there being nothing to place until the Tome is opened.
- **Upgrade Finder: the buildings a Tome can be opened for are weighed too.** They are compared like buildings already in the inventory - an evolving one at the stage your artifacts can reach - and a row that comes out of a Tome names it under the replacement, so you know which one to open. Replace hands you the building to place just as it does for one you hold; opening the Tome for it is yours to do. Grouping by replacement keeps each building of a Tome to itself, and the search box finds a Tome's buildings by its name.

## v10.4.0
- **City Planner: dropping a building on another one takes that one up in its stead.** A drop that landed on an occupied spot used to bounce straight back, so making room meant dragging one building out to a clear patch, moving the second, then fetching the first. Now the building you dropped stays exactly where you put it and the one that was underneath comes up on the cursor, ready to be carried off somewhere else - the drag continues rather than ending. Any drop that covers a single building does this, whatever the two of them measure; only one covering two or more still snaps back, there being no one building to take up. Where the second building goes is yours to decide: put it down without moving and it drops into the space you came from if that space can hold it, and the Move Log records the pair as one entry either way, so Ctrl+Z puts both back.
- **City Planner: a drop that cannot be made now does nothing at all.** Putting a building down where it did not fit either threw it away - that was the fate of one just built, one duplicated, or one a swap had taken up - or dragged it back to where the move began, undoing the work of getting it across the city. Both are a lot to ask of a misplaced click. Such a drop is now simply refused: the building stays on the cursor, exactly where you had it, and you can carry on looking for a spot. Delete is still there for one you have decided against.
...
- **City Planner: a size typed into the search box is found in every view.** The table has long taken a footprint as a search - `7x3` for that exact size, `7x` for anything seven wide, `x3` for anything three long - but the top-down and isometric views put the same box through a search of their own that knew only text, so a size typed there lit up nothing. All three views now go through the one search, so whatever the box accepts in one it accepts in the others. A size is also read as a size rather than as a run of characters: `3x1` finds buildings three wide and one long, and no longer takes a 3x12 or a 13x1 along with them.
- **A building that produces a boosted good names the right one.** The game does not name such a good outright: it sends a rule - so many steps along the chain from the good you are boosted in, at a given tier, as a common, sentient or ascended good - and the extension misread it twice over. It took the game's list of boosted goods to run tier 1, 2, 3, when the game sends tier 3 first, so a tier 1 rule came back with the tier 3 good; and it took the step count to move the tier, when what it moves is which of that tier's three goods is meant - the tier itself never changes. A Tinlug at level 18 pays out Cosmic Bismuth and was said to pay out Platinum. The rule is now read the way the game reads it, which puts the name right in the tooltip, the table, the Upgrade Finder and the Inventory alike.
- **Upgrade Finder: each figure in a resource cell is coloured by its own sign.** The gain per day and the gain per square shared one colour, decided by whether either of them was positive, so a replacement that gains per square while losing in total - or the reverse - showed the loss in the same green as the gain above it. Each is coloured on its own now: green for a gain, red for a loss, and muted where it rounds to nothing.
...
- **City Planner: an evolving building's stage is in its tooltip.** It already drove the culture and population figures shown, but was never named; it now joins the line under the building's name. The isometric view's tooltip was leaving the stage out of the figures altogether, so an evolving building there reported its base culture and population; it reads the stage now.
- **City Planner:** Labels in both views are laid out by one rule, in tiles, so the two can no longer drift apart — and a few things came right with it: a building the catalogue does not know is drawn as a proper block rather than a floating label with nothing to pick up; the warning disc and the max-level tick appear on unlabelled buildings too; and the cursor shows an open hand at rest and a closed one while you carry, not the other way round.
- **Fellowship Adventure:** A badge production you have just started counts down from when you started it. The game reports a production's time left as of the moment it tells you about it, and the only clock the extension kept was the one from when the city loaded — so a production started a moment ago read as though it had been running since then.
- **Chat:** The search row that sticks to the top while you scroll no longer sits over the panel's own menus.

## v10.2.1
- **Safari: the extension ships complete.** The Mac app named the files to carry inside it one by one, in a list written out once and never revisited, so several of the scripts the extension asks for were simply absent — among them the overlay's own injected script and the code the overlay shares with the City tab — and those parts could not work on Safari however recently you had updated. The list is gone: whatever the build produces is what ships, so a file that is added or renamed is carried across on its own, and one that is dropped leaves. Chrome and Firefox were never affected.

## v10.2.0
- **City ▸ Export carries the research tree.** Aimed again at the [Chapter 25 planner](https://paviad.github.io/elvenar-ch25-planner/): the export now holds every technology of the chapter your city is in — its knowledge and goods cost, how much of each is still outstanding, and the edges joining it to its neighbours — so the planner reads the chapter as a graph rather than as a single total. The game only ever names the technologies it has opened, so what is researched, what can take knowledge now and what is still locked behind a predecessor are worked out against the tree itself. `ElvenAssist.researchResources()`, run by hand from the service worker console, prints the same graph as a table with what the chapter's goods have gone on beside what they still owe.
...
- The outline is drawn once the whole grid is down rather than inside the hovered building's own block, so on a tight layout its neighbours no longer paint over most of the border. While a building is being carried the outline stands down — the copy on the cursor already has a ring of its own, and it follows a frame behind the mouse, which had every building it passed over lighting up for a frame. The hover is held by the building being dragged until it lands, and stays on it where it is dropped.
- **A city opens framed on its top-left corner.** The top-down view used to open centred on the grid, which for most cities is a view of empty ground. It now scrolls so that the leftmost and topmost buildings standing on the grid sit five squares in from the edges, and re-frames when you switch to another city.
...
- **Opening a wonder brings its figure back in line.** The push described above only reaches a tab the game is still talking to, and a tab left in the background long enough stops being told — so a wonder's standing could drift while you were away, with nothing short of a reload to settle it. The window you open on the wonder itself asks the server outright, and that answer is now taken as the current figure, however much was missed in the meantime.
...
- **Less of the extension is loaded into the game page.** Two content scripts — one of which runs on every Elvenar page before anything else — were pulling in a 3.65 MB bundle of libraries and using nothing from it. Each is a kilobyte of its own; they now load as such.
- **The build can no longer ship a stale copy of shared code.** The chunk shared between the overlay and the tab had no name of its own, so it was filed under a number that moved whenever the code inside it changed, while three manifests went on naming the old one by hand — and since the output folder was never emptied, the previous build's copy was still there to be loaded. The chunks are named after the parts that share them now, the output is cleared before each build, and every file the manifests and pages name is checked against what the build actually produced.
