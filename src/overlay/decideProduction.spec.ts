...
describe('decide', () => {
  it('leaves a production that is still running alone', () => {
    ...
    expect(decide(producing, fresh(), CITY_LOADED_AT, NOW)).toEqual({ action: 'none', note: 'producing' });
  });
  ...
  it('collects a production whose countdown has run out, even though the game still calls it running', () => {
    ...
    expect(decide(elapsed, fresh(), CITY_LOADED_AT, NOW).action).toBe('pickup');
  });

  it('collects a production the game has reported as finished', () => {
    expect(decide(entity('ProductionFinishedVO'), fresh(), CITY_LOADED_AT, NOW).action).toBe('pickup');
  });

  it('starts an idle building straight away', () => {
    expect(decide(entity('IdleVO'), fresh(), CITY_LOADED_AT, NOW).action).toBe('start');
  });
  ...
  it('starts on the poll after a collect, without waiting for the game to report the idle state', () => {
    ...
    const state = decide(entity('ProductionFinishedVO'), after('pickup', 5_000), CITY_LOADED_AT, NOW);

    expect(state.action).toBe('start');
  });

  it('sends nothing more while it waits for the start to be reported back', () => {
    const state = decide(entity('IdleVO'), after('start', 5_000), CITY_LOADED_AT, NOW);
  ...
  it('starts again when the building is still idle long after the start went out', () => {
    ...
    const state = decide(entity('IdleVO'), after('start', 60_000), CITY_LOADED_AT, NOW);
  ...
  it('collects again when the building is still holding a finished production', () => {
    ...
    const state = decide(entity('ProductionFinishedVO'), after('start', 60_000), CITY_LOADED_AT, NOW);
  ...
  it('does nothing to a building that is doing something else entirely', () => {
    expect(decide(entity('UpgradingVO', 900, NOW), fresh(), CITY_LOADED_AT, NOW)).toEqual({
      action: 'none',
      note: 'is UpgradingVO',
    });
  });
  ...
  it('measures the countdown from the city load when the building has not been reported since', () => {
    ...
    expect(decide(entity('ProducingVO', 120), fresh(), CITY_LOADED_AT, NOW).action).toBe('pickup');
    // 2000s from that same load: still 1000s to go.
    expect(decide(entity('ProducingVO', 2000), fresh(), CITY_LOADED_AT, NOW).note).toBe('producing');
  });
});
