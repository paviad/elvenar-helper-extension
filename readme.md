# Elvenar City Tools Chrome Extension

## Overview

Elvenar City Tools is a Chrome extension designed to enhance your experience with the Elvenar browser game. This extension provides a set of tools to help you plan, optimize, and experiment with your city layout directly from your browser.

### Features

- **City Block Editor**: Easily experiment with your Elvenar city layout using an interactive editor. Move, add, and remove city blocks to find the most efficient and visually appealing arrangement for your city.
- **Future Tools**: This extension is designed to be extensible, with more features planned to help you manage your city, inventory, and resources.

## Installation

1. Download the latest release from this repository, and unpack it into a directory on your computer.
2. Open Chrome and navigate to `chrome://extensions/`.
3. Enable "Developer mode" (top right corner).
4. Click "Load unpacked" and select the directory of the downloaded release.
5. The Elvenar City Tools extension should now appear in your extensions list.

## Usage

- Refresh the Elvenar tab once to allow the extension to get relevant data.
- Click the Elvenar extension icon in your Chrome toolbar to open the city editor.
- Use the city editor to experiment with different city layouts.
- Changes made in the editor are for planning purposes only and do not affect your actual Elvenar city.

## Contributing

Contributions are welcome! Please open an issue or submit a pull request with your suggestions or improvements.

## License

This project is licensed under the MIT License.

---

*Elvenar is a trademark of InnoGames GmbH. This project is not affiliated with or endorsed by InnoGames.*
